...
class ExtResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='cge', num_groups=8, **kwargs):
        ...
        # first convolution
        self.conv1 = SingleConv(in_channels, out_channels, kernel_size=(1,kernel_size,kernel_size), order=order, num_groups=num_groups, padding = (0,1,1))
        # residual block
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(1, 2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='crg',
                 num_groups=8):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 scale_factor=(1, 2, 2), basic_module=DoubleConv, conv_layer_order='crg', num_groups=8):
        super(Decoder, self).__init__()
        if basic_module == ExtResNetBlock:
