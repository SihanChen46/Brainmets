...
from Brainmets.utils import expand_as_one_hot

def regression_accuracy(input, target):
    input = input.round().detach().int().cpu().numpy()
...
def get_loss_criterion(config):
    ...
    if name == 'WeightedBCEWithLogitsLoss':
        skip_last_target = loss_config.get('skip_last_target', False)
        if ignore_index is None and not skip_last_target:
            return nn.BCEWithLogitsLoss(pos_weight=loss_config['pos_weight'])
        else:
            return BCELossWrapper(nn.BCEWithLogitsLoss(pos_weight=loss_config['pos_weight']), ignore_index=ignore_index, skip_last_target=skip_last_target)
...
class FocalLossLogits(nn.Module):
    def __init__(self, pos_weight=1., gamma=0):
    ...
    def forward(self, pred, target):
        logits = self.sigmoid(pred)
        return -torch.mean(((1 - logits)**self.gamma) * target * torch.log(logits) +
                           (1 - target) * (logits ** self.gamma) * (torch.log(1 - logits)))
